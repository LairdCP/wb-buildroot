...
case $1 in
	...
	"uninstall")
		...
		;;
	"tar")
		if [ -f "$ROOTNAME.tar.bz2" ]; then
			echo "File $ROOTNAME.tar.bz2 exists"
			echo -n "Are you sure you want to overwrite $ROOTNAME.tar.bz2? (y/n)"
			read ans
		fi
		if [ "$ans" = "n" ]; then
			exit 0
